...
impl SetLockupChecked<'_> {
    ...
    pub fn invoke_signed(&self, signers: &[Signer]) -> ProgramResult {
        ...
        // - [0..4]: instruction discriminator (u32) = 12
        // - Option<i64> unix_timestamp: 1 byte tag + 8 bytes if Some
        // - Option<u64> epoch: 1 byte tag + 8 bytes if Some
        // Max size: 4 + (1+8) + (1+8) = 22 bytes
        let mut instruction_data = [UNINIT_BYTE; 22];
        let mut offset = 0;

        // Set discriminator as u32 at offset [0..4]
        write_bytes(&mut instruction_data[offset..], &12u32.to_le_bytes());
        offset += 4;
