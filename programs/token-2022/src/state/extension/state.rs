use {
    super::{
        ...
        validate_token_extensions_data, Extension, ExtensionBaseState, ExtensionType,
        ExtensionValue, BASE_ACCOUNT_LEN, TLV_HEADER_LEN, TLV_START_INDEX,
    },
    crate::{
...
pub(super) fn extension_from_bytes<T: Extension>(bytes: &[u8]) -> Result<&T, ProgramError> {
...
#[inline(always)]
#[allow(clippy::manual_is_multiple_of)]
pub(super) fn extension_from_bytes_mut<T: Extension>(
    bytes: &mut [u8],
...
impl ExtensionBaseState for Mint {
    ...
    unsafe fn from_bytes_unchecked_mut(data: &mut [u8]) -> &mut Self {
        &mut *(data[..Self::BASE_LEN].as_mut_ptr() as *mut Mint)
    }
}
...
impl ExtensionBaseState for TokenAccount {
    ...
    #[inline(always)]
    unsafe fn from_bytes_unchecked_mut(data: &mut [u8]) -> &mut Self {
        &mut *(data[..Self::BASE_LEN].as_mut_ptr() as *mut TokenAccount)
    }
}
...
mod tests {
    ...
    fn transfer_hook_account_extension_bool_roundtrip() {
        ...
        extension.transferring = Bool::from(true);
        assert!(bool::from(extension.transferring));

        extension.transferring = Bool::from(false);
        assert!(!bool::from(extension.transferring));
    }
    ...
    fn transfer_hook_extension_write_then_read_roundtrip() {
        ...
        let mint = StateWithExtensions::<Mint>::from_bytes(&data).unwrap();
        let ext = mint.get_extension::<TransferHookExtension>().unwrap();
        assert_eq!(ext.authority, new_authority.into());
        assert_eq!(ext.program_id, new_program_id.into());
    }
}
