...
impl sealed::Sealed for TransferHookAccountExtension {}

// SAFETY: `TransferHookAccountExtension` is repr(C), contains only `Bool`
// which is repr(transparent) over `u8`, has no padding, and all bit patterns
// are valid.
unsafe impl ExtensionValue for TransferHookAccountExtension {
