...
pub fn get_extension_from_bytes<T: Extension + Copy>(ext_bytes: &[u8]) -> Option<T> {
    let mut start = 0;
    let end = ext_bytes.len();
    while start < end {
        let ext_type_idx = start;
        let ext_len_idx = ext_type_idx + 2;
        let ext_data_idx = ext_len_idx + EXTENSION_LEN as usize;
        ...
        if ext_type == T::TYPE && ext_len as usize == T::LEN {
            let ext_data = &ext_bytes[ext_data_idx..ext_data_idx + T::LEN];
            return Some(unsafe { from_bytes(ext_data) });
        }
...
mod tests {
    use crate::{
        extensions::{
            get_extension_from_bytes, transfer_fee::TransferFeeConfig, EXTENSIONS_PADDING,
        },
        state::Mint,
    };
    ...
    fn test_get_extension_from_bytes() {
        ...
        let transfer_fee_ext = get_extension_from_bytes::<TransferFeeConfig>(
            &TEST_MINT_WITH_EXTENSIONS_SLICE[Mint::LEN + EXTENSIONS_PADDING + 1..],
        );
