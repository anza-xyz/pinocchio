    ...
    ApproveChecked<'account, 'multisig, MultisigSigner>
{
    ...
    pub fn invoke_signed(&self, signers: &[Signer]) -> ProgramResult {
        ...
        let mut instruction_accounts =
            [UNINIT_INSTRUCTION_ACCOUNT; ApproveChecked::MAX_ACCOUNTS_LEN];
        let written_instruction_accounts =
            self.write_instruction_accounts(&mut instruction_accounts)?;

        let mut accounts = [UNINIT_CPI_ACCOUNT; ApproveChecked::MAX_ACCOUNTS_LEN];
        let written_accounts = self.write_accounts(&mut accounts)?;

        let mut instruction_data = [UNINIT_BYTE; ApproveChecked::DATA_LEN];
...
fn write_instruction_data(
    ...
    if data.len() < ApproveChecked::DATA_LEN {
        return Err(invalid_argument_error());
    }
