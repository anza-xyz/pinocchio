...
pub unsafe trait DataGuard: sealed::Sealed {
    /// # Safety
    ///
    /// `account` must point to a valid `RuntimeAccount`.
    ...
    /// # Safety
    ///
    /// `size` must equal the value validated by `check_account`. Passing an
    /// incorrect value is undefined behavior.
    #[inline(always)]
    unsafe fn inform_size(&self, _size: usize) {}
...
#[inline(always)]
const fn const_advance_buffer_by(by: usize) -> usize {
    if by == 0 || by % BPF_ALIGN_OF_U128 == 0 {
        STATIC_ACCOUNT_DATA + by
    } else {
        STATIC_ACCOUNT_DATA + by + BPF_ALIGN_OF_U128 - by % BPF_ALIGN_OF_U128
    }
}
...
impl<const N: usize> sealed::Sealed for AssumeSize<N> {}
...
unsafe impl<const N: usize> DataGuard for AssumeSize<N> {
    ...
    unsafe fn advance_buffer(&self, _account: *const RuntimeAccount, buffer: *mut u8) -> *mut u8 {
        buffer.add(const_advance_buffer_by(N))
    }
}
...
impl<T> sealed::Sealed for CheckLikeType<T> {}
...
unsafe impl<T> DataGuard for CheckLikeType<T> {
    ...
    #[inline(always)]
    unsafe fn inform_size(&self, _size: usize) {
        // SAFETY: caller upholds that `_size` equals what `check_account` validated.
        assume_unchecked(
    ...
    #[inline(always)]
    unsafe fn advance_buffer(&self, _account: *const RuntimeAccount, buffer: *mut u8) -> *mut u8 {
        buffer.add(const_advance_buffer_by(core::mem::size_of::<T>()))
    }
}
...
impl<T> sealed::Sealed for AssumeLikeType<T> {}
...
impl InstructionContext {
    ...
    unsafe fn read_account_unchecked<D: DupGuard, S: DataGuard>(
        ...
        let after_header = self.buffer.add(size_of::<u64>());
...
mod tests {
    ...
    #[cfg(feature = "account-resize")]
    const SERIALIZED_NON_DUP_ACCOUNT_LEN: usize = STATIC_ACCOUNT_DATA + size_of::<u64>();

    #[cfg(feature = "account-resize")]
    unsafe fn first_account(input: *mut u8) -> *mut RuntimeAccount {
        input.add(size_of::<u64>()) as *mut RuntimeAccount
    }

    #[cfg(feature = "account-resize")]
    unsafe fn serialized_account_bytes(
        account: *const RuntimeAccount,
    ) -> [u8; SERIALIZED_NON_DUP_ACCOUNT_LEN] {
        let mut bytes = [0u8; SERIALIZED_NON_DUP_ACCOUNT_LEN];
        let len = bytes.len();
        bytes.copy_from_slice(core::slice::from_raw_parts(account as *const u8, len));
        bytes
    }
    ...
    #[cfg(feature = "account-resize")]
    #[test]
    fn test_account_resize_padding_set_by_next_account() {
        let mut input =
            unsafe { create_input_custom(&[AccountDesc::NonDup { data_len: 48 }], &IX_DATA) };
        let account = unsafe { first_account(input.as_mut_ptr()) };
        assert_eq!(unsafe { (*account).padding }, [0; 4]);

        let mut ctx = unsafe { InstructionContext::new_unchecked(input.as_mut_ptr()) };
        let acct = ctx.next_account().unwrap().assume_account();

        assert_eq!(acct.data_len(), 48);
        assert_eq!(unsafe { (*account).padding }, 48u32.to_le_bytes());
    }

    #[cfg(feature = "account-resize")]
    #[test]
    fn test_account_resize_padding_set_by_guarded_typed_account() {
        let type_size = size_of::<MyType>();
        let mut input = unsafe {
            create_input_custom(
                &[AccountDesc::NonDup {
                    data_len: type_size,
        ...
        let mut ctx = unsafe { InstructionContext::new_unchecked(input.as_mut_ptr()) };
        let acct = ctx
            .next_account_guarded(&CheckNonDup, &CheckLikeType::<MyType>::new())
            .unwrap();

        assert_eq!(acct.data_len(), type_size);
        assert_eq!(
            unsafe { (*account).padding },
            (type_size as u32).to_le_bytes()
        );
    }
    ...
    #[cfg(feature = "account-resize")]
    #[test]
    fn test_account_resize_size_error_leaves_account_bytes_untouched() {
        let mut input =
            unsafe { create_input_custom(&[AccountDesc::NonDup { data_len: 32 }], &IX_DATA) };
        let account = unsafe { first_account(input.as_mut_ptr()) };
        let before = unsafe { serialized_account_bytes(account) };
        let mut ctx = unsafe { InstructionContext::new_unchecked(input.as_mut_ptr()) };
        ...
        assert_eq!(unsafe { serialized_account_bytes(account) }, before);
    }
}
